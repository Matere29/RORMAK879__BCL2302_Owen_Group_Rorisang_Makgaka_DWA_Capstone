...
export default function Favorites(props) {
    ...
    function sortByLatest() {
        ...
        setSortPhase(favouriteData.sort((a, b) => new Date(a.Added) - new Date(b.Added)))
    }
    function sortByOldest() {
        setSortState('sortByOldest')
        setSortPhase(favouriteData.sort((a, b) => new Date(b.Added) - new Date(a.Added)))
    }
            ...
            <div className="favouriteSection">
                <h1 className="favTitle">FAVOURITES</h1>
                {
                    state === 'loading' ? <div className="NoFavouritesText">{"LOADING..."}</div> :
                        favouriteData.length === 0 ? <h1 className="NoFavouritesText">No Favourites Available</h1> :
