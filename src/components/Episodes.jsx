...
export default function Episodes(props) {
   ...
   const SaveToFavoritesDatabase = async (EpisodeTitle, EpisodeNumber, EpisodeDescription, EpisodeFile) =>{
   ...
   const SaveToHistoryDatabase = async (EpisodeTitle, EpisodeNumber, EpisodeDescription, EpisodeFile) =>{
    const Season = props.favouriteSeasonTitle
    const Show = props.favouriteShowTitle
    const Email = props.email
    try {
    ...
    const episodeElements = props.Preview.map(item => {
        return (
            ...
            >
                ...
                <audio src={item.file} title={item.title} onPlay={() => SaveToHistoryDatabase(item.title, item.episode, item.description, item.file)} controls/>
                <button onClick={() => SaveToFavoritesDatabase(item.title, item.episode, item.description, item.file)}>Add To Favorites</button>
            </div>
        )
    })
